...
1. **Master DB ingestion (5 of 8 mappable slugs).** `gdaley/hkracing`, `sheikhbarabas`, `takamotoki`, `felipetappata`, `lantanacamara` ingested via the standard Phase-0 pipeline. Three slugs 403'd (joebeachcapital, zygmunt, gdaley/horseracing-in-hk).
2. **User hand-staged gdaley/horseracing-in-hk + zygmunt + an unidentified third (HorseRacing.csv).** Only horseracing-in-hk was integrated this session — the other two would need new preprocessors / field-map rewrites. `gdaley/hkracing` was wiped from master.db and **replaced** by horseracing-in-hk (same 79K races, plus sectional pace data).
3. **`scripts/db/field_maps.py`** — gdaley/horseracing-in-hk entry expanded to map `time1→fraction_q1_sec`, `time2→fraction_q2_sec`, `behind_sec1→beaten_lengths_q1`, `behind_sec2→beaten_lengths_q2`, plus `preprocess: gdaley_horseracing_in_hk_merge`.
4. **`scripts/db/preprocessors.py`** — new `gdaley_horseracing_in_hk_merge` function.
5. **`scripts/db/export_training_data.py`** — `horses.dedup_key AS horse_dedup_key` added to the SELECT so SequenceModel can group by a globally-unique horse ID (per ADR-027 superseded).
6. **`app/services/models/sequence.py`** — Full Transformer encoder. Optional torch dep, MPS-aware device selection, lazy module-level imports so a torch-less install still resolves. ADR-046.
7. **`app/services/models/pace_scenario.py`** — Full LightGBM model with derived per-row + field-relative features. Row-count gate (≥5 000 non-null) instead of percentage. 0.5 fallback on rows without pace data. ADR-047.
8. **Provenance plumbing**: new `app/schemas/provenance.py::ModelProvenance`; added field to `RaceCard`, `Portfolio`, `ParetoFrontier`. Both bootstrap scripts write `BOOTSTRAP_PROVENANCE.json`. Frontend types + `ProvenanceBanner` component + mock data updated.
9. **`scripts/bootstrap_models.py`** — Restructured: 3-way time split (train/calib/test, ADR-003), train sub-models on train slice with calib as early-stop, fit meta-learner on train_stacked w/ calib_stacked val, fit calibrator on meta_pred(calib), eval test-slice ECE/Brier/log-loss pre- AND post-calibration. Writes provenance file.
10. **`tests/conftest.py`** — `KMP_DUPLICATE_LIB_OK=TRUE` + `OMP_NUM_THREADS=1` set to suppress torch+lightgbm OpenMP segfault that fired when both libraries loaded in one pytest process.
11. **`scripts/rolling_retrain.py`** — Pace + Sequence branches updated to fit when `is_trainable_with` returns True (mirrors bootstrap behaviour).
